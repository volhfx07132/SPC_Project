...
// SpcMakerKashi is MasterChef's left hand and kinda a wizard. He can cook up Spc from pretty much anything!
// This contract handles "serving up" rewards for xSpc holders by trading tokens collected from Kashi fees for Spc.
contract SpcMakerKashi is Ownable {
    ...
    function _convertStep(address token0, uint256 amount0) private returns (uint256 SpcOut) {
        if (token0 == Spc) {
            IERC20(token0).safeTransfer(bar, amount0);
            SpcOut = amount0;
        } else if (token0 == weth) {
            SpcOut = _swap(token0, Spc, amount0, bar);
        } else {
            ...
            SpcOut = _convertStep(bridge, amountOut);
        }
    }
