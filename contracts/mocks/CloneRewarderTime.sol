...
contract CloneRewarderTime is IRewarder,  BoringOwnable{
    ...
    /// @notice sender 
    function onSpcReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyMCV2 lock override external {
...
+++
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        // Get number token Spc swap per imcremen per secound
        ...
        // Calculate token suhsi to give to user staking  
            uint256 SpcReward = time.mul(rewardPerSecond);
        // Calculate tokenReward per second    
            accToken1PerShare = accToken1PerShare.add(SpcReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
        // Return pending token can access (Calculate Spc reward token)
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                ...
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                // Get number token Spc rewardPerSecond affter time
                uint256 SpcReward = time.mul(rewardPerSecond);
                // Reset accTOken1PerShare of pool information
                pool.accToken1PerShare = pool.accToken1PerShare.add((SpcReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
